...
class Output_Corr (object):
    ...
    def SMG_Model(self):
        ...
        SMG_div_y_div, self.SMG_div_z_div = mdata.SMG()
        ...
        self.corr_smg[2] = self.corr_3d(self.SMG_div_z_div,self.sz_div)
...
#        corr_fsgs[8] = self.corr_3d(SMG_zz,self.szz)    
        return corr_tfsgs, self.TFSGS_zz  #s_tfL_z  #self.TFSGS_zz     #, dissipation_mean

    def Two_PointCorr_main(self,add_in,t1,t2,t3,t4,ld,Vprime): #
        ##,t1,t2,t3,t4):
        ...
        L11,Lsmg11, Ltfsgs1, Ltfsgs2, Ltfsgs3, Ltfsgs4 = get_TwoPointCorr_DNS(add_in,self.szz,self.SMG_zz,t1,t2,t3,t4,self.redsz,2,ld,Vprime)
        # L11,Lsmg11, Ltfsgs1 = get_TwoPointCorr_DNS(add_in,self.szz,self.SMG_zz,t1,self.redsz,2,ld,Vprime)       
        #L11,Lsmg11, Ltfsgs1, Ltfsgs2, Ltfsgs3, Ltfsgs4 = get_TwoPointCorr_DNS(add_in,self.sz_div,self.SMG_div_z_div,t1,t2,t3,t4,self.redsz,2,ld)
        
        # L11,Lsmg11 = get_TwoPointCorr_DNS(add_in,self.szz,self.SMG_zz,self.redsz,2,ld)
        ...
        #   # Lfsgs11 = get_TwoPointCorr(self.vxbar,self.fl_sxx,self.redsz,0)/np.mean(self.vxbar**3.0)
        return L11, Lsmg11 ,Ltfsgs1 , Ltfsgs2 , Ltfsgs3, Ltfsgs4 #abs(L11)/np.max(abs(L11)), abs(Lsmg11)/np.max(abs(L11)) ,abs(Ltfsgs1)/np.max(abs(Ltfsgs1)) ,\
            #abs(Ltfsgs2)/np.max(abs(Ltfsgs2)), abs(Ltfsgs3)/np.max(abs(Ltfsgs3)), abs(Ltfsgs4)/np.max(abs(Ltfsgs4)) 
             #
