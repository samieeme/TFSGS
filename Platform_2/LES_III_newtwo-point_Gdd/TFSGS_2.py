...
Vprime = []
for i in range(120):
    vz_temp = np.genfromtxt('Prime_Vz_'+str(i)+'_Ensemble.csv')
    vz_ens=vz_temp.reshape(N,N,N)
    Vprime.append(vz_ens)
Vel_av = []
Diss_SMG=[]
Diss1_TFSGS=[]
Diss2_TFSGS=[]
...
for i in range(30):
    ...
    Rs = str(6) #(0.635,0.07,nu,2)(0.65,0.1,nu,2); (0.68,0.00000001,nu,2); [(- DDNS_M) / DTFSGS1_M /(2*int(Rs)**2*(2*np.pi/310)**2)*0.0115]
    solver = Output_Corr(add_in,Rs)
    solver.V_ZeroMean(vx_ens,vy_ens,vz_ens)
    corr_smg,diss_SMG,diss_DNS = solver.SMG_Model()
    corr1_tfsgs, diss1_TFSGS = solver.TFSGS_Model(0.81,0.0000001,nu,2)
    # # LFSGS1 = solver.Two_PointCorr_TFSGS()
    # # # C_SMG = diss_DNS/diss_SMG/(2*int(Rs)**2*(2*np.pi/310)**2)*0.016
    corr2_tfsgs,diss2_TFSGS = solver.TFSGS_Model(0.81,0.02,nu,2)
    # # LFSGS2 = solver.Two_PointCorr_TFSGS()
    corr3_tfsgs,diss3_TFSGS = solver.TFSGS_Model(0.81,0.8,nu,2)
    # # LFSGS3 = solver.Two_PointCorr_TFSGS()
    corr4_tfsgs,diss4_TFSGS = solver.TFSGS_Model(0.81,3,nu,2)
    ...
    Lnn, Lsmg,LFSGS1,LFSGS2,LFSGS3,LFSGS4 = solver.Two_PointCorr_main(add_in,diss1_TFSGS,diss2_TFSGS,diss3_TFSGS,diss4_TFSGS,Rs,Vprime)
    # Lnn, Lsmg,LFSGS1 = solver.Two_PointCorr_main(add_in,diss1_TFSGS,Rs,Vprime)
    #Lnn, Lsmg = solver.Two_PointCorr_main(add_in,Rs) ##,diss1_TFSGS,diss2_TFSGS,diss3_TFSGS,diss4_TFSGS)    
...
Nmax = 140
Nmin = 0
Lnn = abs(sum(DNS_Vel_Stress_Corr[:]))/10#/abs(DDNS_M)
# Lnn=Lnn1-Lnn1[0]
R = np.arange(ns)/8
...
Lsmg = abs(sum(SMG_Vel_Stress_Corr)/10)#/abs(DSMG_M) #SMG_Vel_Stress_Corr[9] #
LFSGS1 = abs(sum(TFSGS_Vel_Stress_Corr1)/10)#/abs(DTFSGS1_M)/10
LFSGS2 = abs(sum(TFSGS_Vel_Stress_Corr2)/10)#/abs(DTFSGS2_M)/10
LFSGS3 = abs(sum(TFSGS_Vel_Stress_Corr3)/10)#/abs(DTFSGS3_M)/10
LFSGS4 = abs(sum(TFSGS_Vel_Stress_Corr4)/10)#/abs(DTFSGS4_M)/10
...
C_DNS = 1.0/Lnn[0] #1/np.max(Lnn)#
C_SMG = 1/Lsmg[0] #1/np.max(Lsmg) #
C_alpha_1 = 1/LFSGS1[0] #1/np.max(LFSGS1) ##(- DDNS_M) / DTFSGS1_M /(2*int(Rs)**2*(2*np.pi/310)**2)*0.015/5/1.10
C_alpha_2 = 1/LFSGS2[0] #1/np.max(LFSGS2) # #(- DDNS_M) / DTFSGS2_M /(2*int(Rs)**2*(2*np.pi/310)**2)*0.015/5/1.10
C_alpha_3 = 1/LFSGS3[0] #1/np.max(LFSGS3) # #(- DDNS_M) / DTFSGS3_M /(2*int(Rs)**2*(2*np.pi/310)**2)*0.015/5/1.10
C_alpha_4 = 1/LFSGS4[0] #1/np.max(LFSGS4) #/9.9 #(- DDNS_M) / DTFSGS4_M /(2*int(Rs)**2*(2*np.pi/310)**2)*0.015/5/1.10
...
######################  Semi-Log X: Small r
plt.plot(R[Nmin:Nmax],C_DNS*Lnn[Nmin:Nmax],color='black', linestyle='solid', linewidth=3, marker='*',
      markerfacecolor='black', markersize=11, label='Filtered DNS')
plt.plot(R[Nmin:Nmax],C_SMG*Lsmg[Nmin:Nmax], color='blue', linestyle='-.', linewidth=3, marker='+',
      markerfacecolor='green', markersize=16, label='SMG Model')

alphaa=0.8
plt.plot(R[Nmin:Nmax],C_alpha_1*LFSGS1[Nmin:Nmax], color='maroon', linestyle='dashed', linewidth=3, marker='s',
        markerfacecolor='maroon', markersize=6, label='FSGS,   $\\alpha = $'+str(alphaa))
plt.plot(R[Nmin:Nmax],C_alpha_2*LFSGS2[Nmin:Nmax], color='crimson', linestyle='dashed', linewidth=3, marker='v',
      markerfacecolor='crimson', markersize=6, label='TFSGS, $\\alpha = $'+str(alphaa)+', $\\lambda = $'+str(0.05))
plt.plot(R[Nmin:Nmax],C_alpha_3*LFSGS3[Nmin:Nmax], color='salmon', linestyle='dashed', linewidth=3, marker='o',
        markerfacecolor='salmon', markersize=6, label='TFSGS, $\\alpha = $'+str(alphaa)+', $\\lambda = $'+str(0.8))
plt.plot(R[Nmin:Nmax],C_alpha_4*LFSGS4[Nmin:Nmax], color='teal', linestyle='dashed', linewidth=3, marker='d',
        markerfacecolor='teal', markersize=6, label='TFSGS, $\\alpha = $'+str(alphaa)+', $\\lambda = $'+str(3))
...
plt.legend(loc='middle right', fontsize=25)
