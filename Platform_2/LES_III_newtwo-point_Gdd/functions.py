...
def get_TwoPointCorr_DNS(add_in,b,b2,t1,t2,t3,t4,N,ax,ld,Vprime): #
    Lnn = np.array([]) 
    Lsmg = np.array([]) 
    Ltfsgs1 = np.array([]) 
    Ltfsgs2 = np.array([]) 
    ...
    for i in range(120):
        ...
        aa = np.transpose(a.reshape(N,N,N))
        aa = aa - Vprime[i]
        #a3 = deriv_z(N,aa)
        Lnn = np.append(Lnn,np.mean(aa*b))
        Lsmg = np.append(Lsmg,np.mean(aa*b2))
        Ltfsgs1 = np.append(Ltfsgs1,np.mean(aa*t1))
        Ltfsgs2 = np.append(Ltfsgs2,np.mean(aa*t2))
        Ltfsgs3 = np.append(Ltfsgs3,np.mean(aa*t3))
        Ltfsgs4 = np.append(Ltfsgs4,np.mean(aa*t4))
