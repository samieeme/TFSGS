...
for i in range(20):
    ...
    corr1_tfsgs, diss1_TFSGS = solver.TFSGS_Model(0.51,0.0000001,nu,2)
    # LFSGS1 = solver.Two_PointCorr_TFSGS()
    # # C_SMG = diss_DNS/diss_SMG/(2*int(Rs)**2*(2*np.pi/310)**2)*0.016
    corr2_tfsgs,diss2_TFSGS = solver.TFSGS_Model(0.51,0.08,nu,2)
    # LFSGS2 = solver.Two_PointCorr_TFSGS()
    corr3_tfsgs,diss3_TFSGS = solver.TFSGS_Model(0.51,0.9,nu,2)
    # LFSGS3 = solver.Two_PointCorr_TFSGS()
    corr4_tfsgs,diss4_TFSGS = solver.TFSGS_Model(0.51,5,nu,2)
    ...
    Lnn, Lsmg,LFSGS1,LFSGS2,LFSGS3,LFSGS4 = solver.Two_PointCorr_main(add_in,diss1_TFSGS,diss2_TFSGS,diss3_TFSGS,diss4_TFSGS,Rs)
...
# ax = fig.add_subplot()
# ax2 = fig.add_axes([0.3, 0.6, 0.2, 0.2])
...
plt.plot(R[Nmin:Nmax],C_SMG*Lsmg[Nmin:Nmax], color='blue', linestyle='-.', linewidth=3, marker='+',
      markerfacecolor='green', markersize=12, label='SMG Model') 

alphaa = 0.6
plt.plot(R[Nmin:Nmax],C_alpha_1*LFSGS1[Nmin:Nmax], color='maroon', linestyle='dashed', linewidth=3, marker='s',
        markerfacecolor='maroon', markersize=9, label='FSGS, $\\alpha = $'+str(alphaa))
plt.plot(R[Nmin:Nmax],C_alpha_2*LFSGS2[Nmin:Nmax], color='crimson', linestyle='dashed', linewidth=3, marker='v',
      markerfacecolor='crimson', markersize=7, label='TFSGS, $\\alpha = $'+str(alphaa)+', $\\lambda = $'+str(0.08))
plt.plot(R[Nmin:Nmax],C_alpha_3*LFSGS3[Nmin:Nmax], color='salmon', linestyle='dashed', linewidth=3, marker='o',
        markerfacecolor='salmon', markersize=7, label='TFSGS, $\\alpha = $'+str(alphaa)+', $\\lambda = $'+str(0.8))
plt.plot(R[Nmin:Nmax],C_alpha_4*LFSGS4[Nmin:Nmax], color='teal', linestyle='dashed', linewidth=3, marker='d',
        markerfacecolor='teal', markersize=7, label='TFSGS, $\\alpha = $'+str(alphaa)+', $\\lambda = $'+str(5))
