...
from functions import remain, deriv_x, deriv_y, deriv_z, div, Reduce_period, Reduce_period_JHU, Rz_deriv_x, Rz_deriv_y, Rz_deriv_z
...
#    def __init__(self,add_in,filename,Rs):
    def __init__(self,add_in,Rs):
        ...
        Vx = np.genfromtxt(add_in+"/"+name_V)
        name_V = "/2_vely-R"+Rs+".csv"
        Vy = np.genfromtxt(add_in+"/"+name_V)
        print(Vy[0])
        ...
        SGSm[:,0] = np.genfromtxt(add_in+"/"+name_sgs)
        name_sgs = "/5_Sxy-R"+Rs+".csv"
        SGSm[:,1] = np.genfromtxt(add_in+"/"+name_sgs)
        name_sgs = "/6_Sxz-R"+Rs+".csv"
        SGSm[:,2] = np.genfromtxt(add_in+"/"+name_sgs)
        name_sgs = "/7_Syy-R"+Rs+".csv"
        SGSm[:,3] = np.genfromtxt(add_in+"/"+name_sgs)
        name_sgs = "/8_Syz-R"+Rs+".csv"
        SGSm[:,4] = np.genfromtxt(add_in+"/"+name_sgs)
        name_sgs = "/9_Szz-R"+Rs+".csv"
        SGSm[:,5] = np.genfromtxt(add_in+"/"+name_sgs)
        print(SGSm[:,5][0])
    # def __init__(self,filename,Rs):
    #     #name_V = filename
    #     name_V = "/1_velx-R"+Rs+".csv"
    ...
    def Output(self):      
        ...
        self.syz = Reduce_period(self.syzm,self.matsz) #- self.vybar * self.vzbar
        self.szz = Reduce_period(self.szzm,self.matsz) #- self.vzbar * self.vzbar
        aa = (self.sxx + self.syy + self.szz)/3
        self.sxx = self.sxx - aa
        self.syy = self.syy - aa
        self.szz = self.szz - aa
        return self.vxbar, self.vybar, self.vzbar, self.sxx, self.sxy, self.sxz, self.syy, self.syz, self.szz, self.matsz
...
class Tempered_FSGS (Pre_Models):
    ...
    def Tempered_Fractional_Laplacian(self,alpha,Lambda,nu):
        ...
        vy = self.vyhat[:]
        vz = self.vzhat[:]
        self.s_fL_x = -mu * self.Tempered_Fractional_Laplacian_2(vx,alpha,Lambda)
        self.s_fL_y = -mu * self.Tempered_Fractional_Laplacian_2(vy,alpha,Lambda)
        self.s_fL_z = -mu * self.Tempered_Fractional_Laplacian_2(vz,alpha,Lambda)
        return self.s_fL_x,self.s_fL_y,self.s_fL_z
    
    def TFSGS_stress (self,a,b,c):
        sxx = -Rz_deriv_x(self.Nnod,a)
        syy = -Rz_deriv_y(self.Nnod,b)
        szz = -Rz_deriv_z(self.Nnod,c)
        sxy = -1.0/2.0*(Rz_deriv_x(self.Nnod,b)+Rz_deriv_y(self.Nnod,a))
        sxz = -1.0/2.0*(Rz_deriv_x(self.Nnod,c)+Rz_deriv_z(self.Nnod,a))
        syz = -1.0/2.0*(Rz_deriv_z(self.Nnod,b)+Rz_deriv_y(self.Nnod,c))
        return sxx, syy, szz, sxy, sxz, syz
