...
for i in range(num_threads):    
    if i == 0:
        image_part[i] = np.concatenate((np.reshape(image[N-R:,:],(R,N)),image[0:int(N/num_threads)+R,:]),axis=0)
    elif i == num_threads-1:
        image_part[i] = np.concatenate((image[i*int(N/num_threads)-R:N,:],np.reshape(image[0:R,:],(R,N))),axis=0)
    else:
...
for i in range(num_threads):
    for j in range(num_threads):
        if (i==im_all_filtered[j][int(N/num_threads)-1+2*R+1,0]):
...
# np.max(np.abs(image_filtered_part-image_filtered))
